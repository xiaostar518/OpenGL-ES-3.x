...
public class MyActivity extends Activity {
	...
	TextView distance;	//TextView对象引用	
	TextView info;	
    @Override
    public void onCreate(Bundle savedInstanceState) {
        ...
        distance = (TextView)findViewById(R.id.distance);	//用于显示距离的TextView
	...
	private SensorEventListener mySensorListener = 
		...
		@Override
		public void onAccuracyChanged(Sensor sensor, int accuracy){}//重写onSensorChanged方法
		@Override
		public void onSensorChanged(SensorEvent event){
			float []values=event.values;//获取距离值所属的values数组
			distance.setText("距离为："+values[0]);//显示距离			
		}
	};
