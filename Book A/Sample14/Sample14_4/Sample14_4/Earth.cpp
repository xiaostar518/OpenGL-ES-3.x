...
void Earth::initVertexData(float r)
{
    ...
    static float tex[7776];//声明纹理坐标数组
    generateTexCoor(36, 18, tex);//生成纹理坐标
    mTexCoorBuffer = tex;
}
...
void Earth::generateTexCoor(int bw,int bh,float* tex){
    ...
    int c=0;
    for(int i=0;i<bh;i++){//遍历列
        for(int j=0;j<bw;j++){//遍历行
...
{    
    mProgram = ShaderUtil::createProgram("Simplevert", "Simplefrag");
...
void Earth::drawSelf(GLint texId,GLint texIdNight)
{
    ...
    glBindTexture(GL_TEXTURE_2D, texId);   //绑定白天纹理
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, texIdNight);//绑定黑夜纹理
